...
import { EmptyState, Icon, Panel, SectionHeading, StatCard, WorkspacePage } from '../ui/ui';
  ...
  return (
    <WorkspacePage>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          label="Flagged"
          value={progress.flaggedImages}
          detail="Copied to flagged output"
          tone={progress.flaggedImages ? 'warning' : 'neutral'}
        />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <MediaList
          title="Images in source"
          icon="image"
          items={images.map((file) => file.name)}
          busy={busy}
          empty="No supported images scanned."
        />
        <MediaList
          title="Videos in source"
          icon="video"
          items={videos.map((file) => file.name)}
          busy={busy}
          empty="No supported videos scanned."
        />
      </div>
      ...
      <WorkflowProgressCard
        ...
        currentStep={
          progress.currentStep === 'classifying'
            ? 'Classifying media'
            : progress.currentStep === 'extracting'
      ...
      {busy ? (
        ...
        <div className="mt-3 max-h-44 space-y-1 overflow-y-auto pr-1">
          {items.map((name) => (
            <div
              key={name}
              className="truncate rounded border border-surface-border bg-surface px-2.5 py-1.5 font-mono text-xs text-slate-400"
              title={name}
            >
              {name}
            </div>
